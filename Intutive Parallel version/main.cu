...
int main(int argc, char**argv)
{
    ...
    // variabili per GPU
    float *Bdata,*Gdata,*Rdata, *nValue;
    cudaMalloc(&Bdata,n_threads*sizeof(float));
    cudaMalloc(&Gdata,n_threads*sizeof(float));
    cudaMalloc(&Rdata,n_threads*sizeof(float));
    cudaMalloc(&nValue,n_threads*sizeof(float));
    ...
    while (error > 0){
        ...
        {
            update_cluster<<<1,n_threads>>>(clust, centr, B_c, G_c, R_c, size_image, n_threads, i/3, Bdata, Gdata, Rdata, nValue);
            cudaThreadSynchronize();
        }
