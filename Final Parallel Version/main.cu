...
int main(int argc, char**argv)
{
    ...
    float *B_count, *G_count, *R_count, *nValue;
    cudaMalloc(&B_count, k*n_threads* sizeof(float));
    cudaMalloc(&G_count, k*n_threads* sizeof(float));
    cudaMalloc(&R_count, k*n_threads* sizeof(float));
    cudaMalloc(&nValue, k*n_threads* sizeof(float));
    ...
    while (error > 0){
        ...
        update_cluster<<<k,n_threads>>>(clust, centr, B_c, G_c, R_c, size_image, n_threads, k, B_count, G_count, R_count, nValue);
        cudaThreadSynchronize();
